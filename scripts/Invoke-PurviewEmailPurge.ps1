...
param(
    ...
    [ValidateSet("ReceivedOrSent", "Received", "Sent")]
    [string]$DateField = "ReceivedOrSent",
...
if ([string]::IsNullOrWhiteSpace($SearchName)) {
    ...
    $SearchName = "OutlookTS-EmailPurge-$safeMailbox"
}

$dateQuery = switch ($DateField) {
    "Received" { 'received>="{0}" AND received<"{1}"' -f $StartDate.ToString("yyyy-MM-dd"), $cutoff.ToString("yyyy-MM-dd") }
    "Sent" { 'sent>="{0}" AND sent<"{1}"' -f $StartDate.ToString("yyyy-MM-dd"), $cutoff.ToString("yyyy-MM-dd") }
    default {
        '(received>="{0}" AND received<"{1}") OR (sent>="{0}" AND sent<"{1}")' -f $StartDate.ToString("yyyy-MM-dd"), $cutoff.ToString("yyyy-MM-dd")
    }
}
...
if (-not [string]::IsNullOrWhiteSpace($FolderName)) {
    ...
    if ($null -eq $folderStats) {
        throw "Could not find folder '$FolderName' for $Mailbox."
    }

    $purviewFolderId = Convert-MailboxFolderIdToPurviewFolderId -FolderId $folderStats.FolderId
    $folderQuery = "folderid:$purviewFolderId"
...
$queryParts = @("kind:email", "($dateQuery)")
if ($folderQuery) {
...
if ($PSCmdlet.ShouldProcess($SearchName, "Create targeted Purview email compliance search")) {
...
Write-Host "Matched email items older than $($cutoff.ToString("yyyy-MM-dd")): $remainingItems" -ForegroundColor Green
...
do {
    ...
    if ($PSCmdlet.ShouldProcess($SearchName, "Run Purview email purge pass $pass using $PurgeType")) {
...
Write-Host "Success. All targeted email items older than $($cutoff.ToString("yyyy-MM-dd")) were purged from $Mailbox." -ForegroundColor Green
